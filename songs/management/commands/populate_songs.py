...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if r.status_code == requests.codes.ok:
            ...
            self.stdout.write(
                self.style.ERROR(f"I cant get songs... status_code={r.status_code}")
            )
            sys.exit(1)
        ...
        for order, song in enumerate(data["feed"]["results"], start=1):
            ...
            instance.artist, _ = Artist.objects.get_or_create(
                id=song["artistId"],
                defaults={"name": song["artistName"], "url": song["artistUrl"]},
            )
            ...
            genres = [
                Genre.objects.get_or_create(
                    id=genre["genreId"], name=genre["name"], url=genre["url"]
                )
